...
async def search(query: str, per_page: int = 20, page: int = 1) -> dict:
    ...
    params = {"query": query, "per_page": per_page, "page": max(1, int(page)), "orientation": "landscape"}
    ...
    has_more = items and params["page"] < int(j.get("total_pages") or 0)
    return {"items": items, "next": str(params["page"] + 1) if has_more else None}
